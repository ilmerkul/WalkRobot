...
class RobotDescPub(Node):
    ...
    def __init__(self, args):
        ...
        super().__init__("robot_desciption_publisher")
        parser = argparse.ArgumentParser(
            description="Publish into the given topic ( robot_description"
            " by default ) the XML string describing you robot"
        )
        parser.add_argument(
            "-xml_string",
            required=True,
            type=str,
            metavar="XML_URDF_XACRO",
        ...
        parser.add_argument(
            ...
            metavar="ROBOT_DESCRIPTION_TOPIC",
            help="Name of topic where the robot rescription"
            " from xml data will be published",
        )

        self.args = parser.parse_args(args[1:])

        rclpy.logging.set_logger_level(
            "robot_desciption_publisher", rclpy.logging.LoggingSeverity.ERROR
        )
...
def main(args=None):
    ...
    robot_desciption_publisher = RobotDescPub(args_without_ros)

    rclpy.spin(robot_desciption_publisher)

    robot_desciption_publisher.destroy_node()
