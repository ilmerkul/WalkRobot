...
envs = [
    create_environment(ENV_NAME, SEED + seed_c) for seed_c in range(environments_count)
]
...
class MemoryLinear(nn.Module):
    def __init__(self, emb_dim: int, batch_dim: int):
        ...
        self.act = nn.Sigmoid()
        self.memory_matrix = self.act(torch.normal(0, 1, (batch_dim, emb_dim, emb_dim)))
        self.alpha = 0.25
...
class MemoryModule(nn.Module):
    def __init__(self, memory_count: int, emb_dim: int, batch_dim: int):
        ...
        self.memory = [MemoryLinear(emb_dim, batch_dim) for _ in range(memory_count)]
        self.softmax = nn.Softmax()

        self.centroid = torch.concatenate(
            [self._get_centroid(memory).unsqueeze(0) for memory in self.memory], dim=0
        )
...
model = WalkerModel(
    action_dim=action_dim, observation_dim=observation_dim, batch_dim=environments_count
)
...
plt.ion()
fig, (
    (ax_reward, ax_world_model_weight, ax_loss_world_model),
    (ax_loss_actor, ax_loss_critic, _),
) = plt.subplots(2, 3)
...
(line_world_model_weight,) = ax_world_model_weight.plot(history["world_model_weight"])
...
for k in range(100000):
    ...
    for i in range(ROLLOUT_DIM):
        ...
        for j, env in enumerate(envs):
            observation, reward, terminated, truncated, info = env.step(action[j])
    ...
    for i in range(ROLLOUT_DIM - 1):
        ...
        #                                                world_emb_batch[:,
        #                                                i + 1, :]) / norm
        intrinsic_reward[:, i + 1] += torch.mean(
            (world_emb_predict_batch[:, i, :] - world_emb_batch[:, i + 1, :].detach())
            ** 2,
            dim=-1,
        )
    ...
    if k != 0:
        reward_batch += 10 * torch.where(
            intrinsic_reward.detach() > 0.02, 0.02, intrinsic_reward.detach()
        )
    reward_batch -= torch.mean(
        (
            beta_batch
            - torch.fill(torch.empty(environments_count, ROLLOUT_DIM, 3), 1 / 3)
        )
        ** 2,
        dim=-1,
    )
    ...
    loss_actor = -1 * torch.mean(log_prob_batch * (estimated_Q - V_batch.detach()))
    ...
    if k != 0:
        ...
        world_model_weight = (
            0.8 * history["world_model_weight"][-1] + (1 - 0.8) * world_model_weight
        )
        loss_actor = 0.8 * history["loss_actor"][-1] + (1 - 0.8) * loss_actor
        loss_critic = 0.8 * history["loss_critic"][-1] + (1 - 0.8) * loss_critic
        loss_world_model = (
            0.8 * history["loss_world_model"][-1] + (1 - 0.8) * loss_world_model
        )
    ...
    for line, ax, name in [
        (line_reward, ax_reward, "reward"),
        (line_world_model_weight, ax_world_model_weight, "world_model_weight"),
        (line_loss_actor, ax_loss_actor, "loss_actor"),
        (line_loss_critic, ax_loss_critic, "loss_critic"),
        (line_loss_world_model, ax_loss_world_model, "loss_world_model"),
    ]:
