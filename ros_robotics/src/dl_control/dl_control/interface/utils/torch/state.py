...
def agr_obs_to_state(msg: AgrObs) -> State:
    ...
    velocity = [
        velocity[i] if i < len(velocity) else 0.0 for i in range(len(JOINT_ORDER))
    ]
    effort = [velocity[i] if i < len(effort) else 0.0 for i in range(len(JOINT_ORDER))]
...
def get_zero_state() -> State:
    return State(
        ...
        foots_forces=torch.tensor([[0.0 for _ in range(State.dim_foots)]]),
        position=torch.tensor([[0.0 for _ in range(State.dim_joint)]]),
        velocity=torch.tensor([[0.0 for _ in range(State.dim_joint)]]),
        effort=torch.tensor([[0.0 for _ in range(State.dim_joint)]]),
    )
...
def concat_state(states: Iterable[State]) -> State:
    ...
    foots_forces = torch.tensor([[0.0] * State.dim_foots] * batch_dim)
    position = torch.tensor([[[0.0] * State.dim_joint]] * batch_dim)
    velocity = torch.tensor([[[0.0] * State.dim_joint]] * batch_dim)
    effort = torch.tensor([[[0.0] * State.dim_joint]] * batch_dim)
    ...
    effort = torch.concat([state.effort for state in states], dim=0)
...
def quaternion_to_euler(
    ...
    pitch = torch.where(
        torch.abs(sinp) >= 1, torch.sign(sinp) * (torch.pi / 2), torch.arcsin(sinp)
    )
