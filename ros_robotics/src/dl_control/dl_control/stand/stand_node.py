...
from dl_control.interface.utils.torch import agr_obs_to_state
from dl_control.stand.SAC.torch import Agent
...
class StandNode(Node):
    ...
    def _create_pub(self, namespace: str):
    ...
    def observation_callback(self, msg: AgrObs) -> None:
        ...
        if True and mode == "train":
            self.agent.train()
            self.get_logger().info(f"{mode}")
            # if self.call_control_service(control_message=ControlSimMessage.PAUSE_MSG):
            #    self.get_logger().info("Train")
            #    self.agent.train()
            #    self.call_control_service(control_message=ControlSimMessage.UNPAUSE_MSG)
        elif False and mode == "reset":
            if self.call_control_service(control_message=ControlSimMessage.PAUSE_MSG):
                self.get_logger().info("Reset")
                self.call_control_service(control_message=ControlSimMessage.RESET_MSG)
                self.call_control_service(control_message=ControlSimMessage.UNPAUSE_MSG)
    ...
    def call_control_service(
        self, control_message: str, timeout_sec: float = 30.0
    ) -> bool:
        try:
            request = ControlSim.Request()
            request.entity_name = self.get_parameter("entity_name").value
